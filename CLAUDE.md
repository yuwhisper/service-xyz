...
| GET | `/yingdao/apps` | 影刀应用字典；`key` 模糊搜名称；含定时调度机器人绑定 |
| GET | `/yingdao/apps/search` | 按应用名搜索并返回调度机器人候选（启动页） |
| GET | `/yingdao/apps/{robotUuid}/bindings` | 单个应用的定时调度机器人绑定 |
...
- 凭证：`DINGTALK_APP_KEY` / `SECRET`（钉盘另需 `UNION_ID`）
...
- 应用列表：`GET /yingdao/apps?key=&page=&size=`（代理影刀 `robot/v2/query`）；附带定时任务绑定的 `runClients`
- 应用搜索：`GET /yingdao/apps/search?key=&limit=`，供每日数据补全按名称选应用并回填调度机器人
